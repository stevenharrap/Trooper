trooper.ui.control.dateTimePicker = (function (params) {
    ...
    this.init = function () {
	    ...
	    if (current != null && !this.isMomentInRange(current)) {
    ...
    this.updateCalendar = function () {
		...
		for (var i = 1; i<= 42; i++) {
		    ...
		    var isSelectable = isMonth && this.isMomentInRange(startMoment);
	...
	this.userInputChanged = function () {
		...
		if (this.isMomentInRange(result)) {
	...
	this.isMomentInRange = function (value) {	    
	    if (this.minimum == null && this.maximum == null) {
	        return true;
	    } else if (this.minimum == null) {
	        return value.isBefore(this.maximum);
	    } else if (this.maximum == null) {
	        return value.isAfter(this.minimum);
	    } else {
	        return value.isBetween(this.minimum, this.maximum);
	    }	    
	};
	...
	var publicResult = {
	    ...
	    serverFormat: $.proxy(this.serverFormat, this),
	};
