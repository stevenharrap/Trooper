	...
	from 
		...
		cross join 
			...
			from 
				Outcome o
			where 
				o.Action in ('GettingAll', 'All')) as o
	where
...
where
	[Entry] not in (
		'HasItems_ItemsExist_IdentityIsAllowed_ReportsErrorAndNoChange',
		'HasItems_ItemsNonExist_IdentityIsAllowed_ReportsErrorAndNoChange'
	)
order by [Entry]
